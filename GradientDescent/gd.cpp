...
Eigen::VectorXd gd(ObjFunc f, const int max_iter, Eigen::VectorXd int_val, double rel_sol_change_tol,
    ...
    while(iter < max_iter && rel_sol_change > rel_sol_change_tol && grad_change > grad_tol){
        ...
        // L2 norm of the gradient at step to x_i
        grad_change =  df.norm();
        // Then update the steps (x_i's) for the next iteration.
        step_prev = step;
        iter++;
    }
...
int main()
{
    ...
    double grad_tol = pow(10,-10); // Set the tolerance the gradient

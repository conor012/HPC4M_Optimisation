M = readmatrix('QuadBFGStrajectory.csv');
...
%%Initialize video
myVideo = VideoWriter('QuadraticBFGS.mp4','MPEG-4'); %open video file
myVideo.FrameRate = 10;  %can adjust this, 5 - 10 works well for me
open(myVideo)
%% Plot in a loop and grab frames
for i=1:length(x)
    if(abs(z(i) - min(z)) > 1e-4)
        plot3(x(1:i), y(1:i), quadratic(x(1:i),y(1:i)),'r', 'LineWidth', 1.5);
...
%         plot(xbfgs(1:min(i,length(xbfgs))), ybfgs(1:min(i,length(xbfgs))),'b', 'LineWidth', 2);
        pause(0.00001) %Pause and grab frame
